...
def diarize(
    ...
    output_path = f"{os.path.splitext(audio)[0]}.srt"
    if output:
        os.makedirs(output, exist_ok=True)
        output_path = os.path.join(output, f"{os.path.splitext(os.path.basename(audio))[0]}.srt")
    ...
    if stemming:
        ...
        return_code = os.system(
            f'{sys.executable} -m demucs.separate -n htdemucs --two-stems=vocals "{audio}" -o "{temp_outputs_dir}" --device "{device}"'
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-o", "--output", help="directory path where to output srt file, defaults to audio directory"
    )
    parser.add_argument(
        "--no-stem",
        action="store_false",
